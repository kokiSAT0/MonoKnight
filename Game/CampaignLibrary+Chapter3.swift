...
extension CampaignLibrary {
    static func buildChapter3() -> CampaignChapter {
        let stage31 = targetStage(chapter: 3, index: 1, title: "縦横応用", summary: "縦横選択を使い、次の目的地にも残る位置を選びます。", goalCount: 8, deckPreset: .standardWithOrthogonalChoices, secondaryObjective: .finishWithFocusAtMost(maxFocusCount: 3), scoreTarget: 290, unlockRequirement: .stageClear(CampaignStageID(chapter: 2, index: 8)))
        let stage32 = targetStage(chapter: 3, index: 2, title: "斜め応用", summary: "斜め選択で角度を作り、遠回りに見える近道を探します。", goalCount: 8, deckPreset: .standardWithDiagonalChoices, spawnRule: .chooseAnyAfterPreview, secondaryObjective: .finishWithinMoves(maxMoves: 19), scoreTarget: 285, unlockRequirement: .stageClear(stage31.id))
        let stage33 = targetStage(chapter: 3, index: 3, title: "桂馬応用", summary: "桂馬選択で大きく位置を変え、別の表示目的地へ切り替えます。", goalCount: 9, deckPreset: .standardWithKnightChoices, secondaryObjective: .finishWithFocusAtMost(maxFocusCount: 3), scoreTarget: 310, unlockRequirement: .stageClear(stage32.id))
