...
- `DungeonRunState` を `GameMode.DungeonMetadata` に持たせ、HP、総手数、所持カード、ラン seed、成長塔の開始区間、試練塔の手札拡張状態を引き継ぐ
...
- 所持上限は通常カード枠で管理する。成長塔は初期5種類、スキル取得で最大9種類まで増え、試練塔は初期5種類、ラン中の手札拡張で最大9種類まで増える。基本移動が許可されたフロアでは通常枠を消費しない基本移動カードを別枠表示する。同じカードは種類枠を増やさず、残り使用回数として積む
- 基本移動が許可された塔では、カードを消費しない基本移動を許可する。標準騎士は上下左右1マス、跳躍騎士はナイト8方向を使う。試練塔でも騎士選択は成長塔と同じ解放条件に従う
- 複数マス移動は通過マスごとに敵踏み潰し、罠、床崩落、拾得カード、鍵/出口を解決し、表示も一歩ずつ反映する。停止条件が出た時点で残り経路を踏まない
- プレイヤー移動が止まった後は `出口判定 → 敵ターン → 失敗判定` の順で解決する

## 3. Game Center

Game Center は将来の試練塔向け leaderboard 基盤として、認証、スコア送信、leaderboard 表示のサービス境界だけを残す。試練塔のローカル最高到達階は標準騎士/跳躍騎士別に保存するが、現時点では旧モード用 leaderboard ID を設定せず、塔プレイから自動送信や自動サインイン促しを行わない。

## 4. 自動テスト方針

- `DungeonModeTests` / `GameCoreTests` で出口到達、HP 0、手数上限後の疲労、敵危険範囲、床割れ、報酬カード持ち越し、成長塔 seed 変化、試練塔の手札拡張確率と生成物の非重複を確認する
- `DungeonSelectionViewTests` でタイトルから塔選択だけが主導線になること、成長塔の区間解放と成長表示、試練塔の騎士選択と騎士別最高到達表示を確認する
