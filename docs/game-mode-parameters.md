...
- `DungeonFloorDefinition.cardPickups` でフロア内の床落ちカードを定義する。拾得カードは1回使い切りで、そのフロア限り有効。拾得/報酬候補には基本移動と同じ上下左右1マスカードを入れない
- ラン中報酬カードは `DungeonRunState.rewardInventoryEntries` に残り回数つきで保持する。報酬カードは3回使用可能で、残り回数を次フロアへ持ち越す
...
| 新キャンペーン | フロア定義に従う、出口到達 | 塔ダンジョン。HP、手数、敵、床ギミックで失敗条件を作る。 |
| 旧キャンペーン | 8×8、ステージ定義に従う、目的地 3〜14 個 | 移行元の目的地制定義として保持する。 |
