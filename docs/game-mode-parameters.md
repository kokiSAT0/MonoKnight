...
- 所持上限は通常カード枠で管理する。成長塔は初期5種類、スキル取得で最大9種類まで増え、基本移動が許可されたフロアでは通常枠を消費しない基本移動カードを別枠表示する。同じカードは種類枠を増やさず、残り使用回数として積む
...
- `DungeonModeTests` / `GameCoreTests` で出口到達、HP 0、手数上限後の疲労、敵危険範囲、床割れ、報酬カード持ち越し、成長塔 seed 変化を確認する
