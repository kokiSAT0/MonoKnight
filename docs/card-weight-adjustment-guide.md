...
- 遺物は `DungeonWeightedRewardPoolItem.relic` として同じテーブルへ登録する。床拾得ではカテゴリ比率が0なので出ず、クリア報酬では既に所持している遺物を除外する。遺物カテゴリ内ではコモン/レア/レジェンドの希少度重みを使い、手早いクリア時はレア/レジェンドへ寄せる。
...
- `DungeonWeightedRewardPools.entries(floorIndex:context:)` は、1-5F、6-10F、11-15F、16-20F、21-30F、31-40F、41F以降ごとの候補と weight を返す。通常は方向均等化済みの成長塔プロファイルを使い、試練塔では `profile: .rogueTower` の専用カーブを使う。
...
遺物の個別 weight は希少度重みと掛け合わせる。通常クリア報酬は コモン70 / レア25 / レジェンド5、70%以内クリアは 60 / 32 / 8、50%以内クリアは 45 / 40 / 15 を使い、`曇った鏡` がある場合は通常クリア報酬の希少度重みに戻す。

1-5F の補助カード枠には `補給` と `消滅の呪文` を入れる。序盤バランスを崩しやすい `全滅の呪文`、`凍結の呪文`、`障壁の呪文`、`ダークネスの呪文`、`レール破壊の呪文` は 1-5F には入れず、中盤以降の候補として扱う。`ダークネスの呪文` と `レール破壊の呪文` は見張り系や巡回兵が複合する 11F 以降の補助枠へ入れる。

成長塔の移動カードは、床拾得とクリア報酬の両方で方向差を持たせない。既存テーブルの種類カーブは維持し、`straight*2` 4方向、`diagonal*2` 4方向、`ray*` 8方向、`knight*Choice` 4方向の各系統で、1方向でも出る階層帯なら全方向を候補に出す。各方向の weight は、その階層帯と排出種別の同系統内で一番高い既存 weight に揃える。成長塔では試練塔用の `king*DiagonalChoice` は追加しない。

試練塔の移動カードも、床拾得とクリア報酬の両方で方向差を持たせない。単方向は `straight*2` 4方向と `diagonal*2` 4方向、レイは8方向、複数方向はキング斜め選択4方向とナイト選択4方向を使い、同じ系統内の各方向 weight を必ず同じにする。試練塔の移動カード weight は次の専用カーブにする。
...
序盤は単方向を中心にして読みやすい移動を増やし、階層が進むほど複数方向カードを増やしてルート選択を広げる。レイは深層で途中被弾や停止ギミックに弱いため、終盤では低 weight の候補として残す。
