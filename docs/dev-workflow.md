...
ロジック変更は `swift test` を優先する。
UI や外部連携を含む変更は、必要に応じて Xcode ビルド、シミュレーター、実機、TestFlight で確認する。
...
- `swift test`
- `xcodebuild test -project MonoKnight.xcodeproj -scheme MonoKnight -destination 'platform=iOS Simulator,name=iPhone 16,OS=18.6' -only-testing:MonoKnightAppTests/GameViewModelTests`
- `xcodebuild -project MonoKnight.xcodeproj -scheme MonoKnight -destination 'platform=iOS Simulator,name=iPhone 16,OS=18.6' build`
