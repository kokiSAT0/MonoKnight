...
`Scripts/codex-safe-validate.sh app-test` の無指定実行は、端末・scheme・app test 起動確認用の軽い smoke として扱う。
変更箇所に対応する app test は、`app-test MonoKnightAppTests/...` で明示指定する。
この開発端末の標準シミュレーターは `platform=iOS Simulator,name=iPhone 16,OS=18.6` とする。
`OS=latest` は最新ランタイム側に `iPhone 16` が存在しない場合に destination 解決で失敗するため、通常検証では使わない。
...
安全検証スクリプトの既定 destination は `iPhone 16 / iOS 18.6` に固定する。
