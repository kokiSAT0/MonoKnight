...
| 8-1 固定ワープ基礎 | 5×5 / 中央固定 | `fixedWarpSpecialized` | 固定ワープ高頻度＋基礎移動フォロー | 38 手以内 | 400 以下 | +3 / +2 / +1 / 0 | 7-8 拡張行動総決算クリア |
...
| `standardWithAllChoices` | 標準 + 全選択カード 10 種 | 選択カードのみ重み 2 / その他は 1 | 3-4・5-1 の総合演習。 |
| `fixedWarpSpecialized` | 固定ワープ＋基礎移動 9 種 | 固定ワープ重み 5／サポートカード重み 1 | 8-1 固定ワープ基礎。 |
| `superWarpHighFrequency` | 標準セット＋全域ワープ | 全域ワープを重み 4、そのほかは 1 | 8-2 全域ワープ試験。 |
...
| 7-3 障害回避演習 | 5×5 / 任意スポーン | `extendedWithMultiStepMoves` | 障害物 {(2,2)}＋複数マス移動カード | 36 手以内 | 430 以下 | +3 / +2 / +1 / 0 | 7-2 クリア |
...
- 前半（8-1・8-2・8-4）は中央固定スポーンでカード挙動に集中し、中央障害物を配置する 8-3 は任意スポーンで初手詰まりを防ぐ。後半（8-5〜8-8）は任意スポーンで難度を一段引き上げる。<!-- スポーン政策 -->
- 山札プリセットは 8-1 が `fixedWarpSpecialized`、8-2 が `superWarpHighFrequency`、8-3 以降は `.standardWithWarpCards` を使用し、段階的にワープカードの習熟度を高める。<!-- デッキ内容 -->
...
| `spawnRule` | 8-1・8-2・8-4: `.fixed(BoardGeometry.defaultSpawnPoint)` / 8-3・8-5〜8-8: `.chooseAnyAfterPreview`（障害物や自由配置に合わせて任意スポーン化） |
| `deckPreset` | 8-1: `fixedWarpSpecialized` / 8-2: `superWarpHighFrequency` / 8-3〜8-8: `.standardWithWarpCards` |
...
| ステージ | 盤面 / スポーン | 山札プリセット | 特殊タイル / カード | セカンダリ条件 | スコア上限 | ペナルティ設定 | アンロック条件 |
|----------|-----------------|----------------|----------------------|----------------|------------|-----------------|----------------|
| 8-1 固定ワープ基礎 | 5×5 / 中央固定 | `fixedWarpSpecialized` | 固定ワープ高頻度＋キング＆桂馬サポート | 38 手以内 | 400 以下 | +3 / +2 / +1 / 0 | 7-8 拡張行動総決算クリア |
| 8-2 全域ワープ試験 | 5×5 / 中央固定 | `superWarpHighFrequency` | 全域ワープを重み 4 で高頻度供給 | ペナルティ合計 2 以下 | 395 以下 | +3 / +2 / +1 / 0 | 8-1 クリア |
| 8-3 障害物併用演習 | 5×5 / 任意スポーン | `standardWithWarpCards` | 障害物 {(2,2)}＋バランス型ワープデッキ | 36 手以内 | 390 以下 | +3 / +2 / +1 / 0 | 8-2 クリア |
| 8-4 反転連動演習 | 5×5 / 中央固定 | `standardWithWarpCards` | トグル {(1,1),(3,3)}＋バランス型ワープデッキ | 34 手以内 | 385 以下 | +3 / +2 / +1 / 0 | 8-3 クリア |
...
| 8-8 空間支配最終試験 | 5×5 / 任意スポーン | `standardWithWarpCards` | バランス型ワープカード群＋トグル {(1,3)}＋二度踏み {(3,1):2}＋障害物 {(0,0)} | ペナルティ合計 1 以下＆30 手以内 | 365 以下 | +3 / +2 / +1 / 0 | 8-7 クリア |
