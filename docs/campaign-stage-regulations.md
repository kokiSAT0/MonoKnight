...
* ワープマスを中心に、盤面上の空間的制約を解放する章。移動経路の概念を再定義することで、従来の踏破順序に新しい戦略性を与える。
* ワープは常にペアで存在し、片方に到達するともう片方へ自動転送される。同時に両方のマスが踏破済みになる。
* 盤面はすべて5×5。スポーンは前半が中央固定、後半に任意スポーンを導入。
* ペナルティ設定は標準準拠（+3 / +2 / +1 / 0）。

実装デザイン仕様

ビジュアル: 魔法陣型。マスの四隅に小さな三角形マーカーを配置し、中心に淡い青紫の発光円を描画。踏破時は渦巻きエフェクト（SKEmitterNode で粒子回転）。

アニメーション: ペアのマス間で1フレームの光線リンクを描写（SKShapeNode ラインアニメーション）。
...
| ----------------- | ------------------------------------------------------------------------------------------------------ |
| `boardSize`       | 常に 5                                                                                                   |
| `spawnRule`       | 6-1〜6-4: `.fixed(BoardGeometry.defaultSpawnPoint)` / 6-5〜6-7: `.chooseAnyAfterPreview` / 6-8: `.fixed` |
| `deckPreset`      | `.standardWithAllChoices`                                                                              |
| `penaltySettings` | `.init(deadlock: 3, manualRedraw: 2, manualDiscard: 1, revisit: 0)`                                    |
...
| ステージ         | 盤面 / スポーン  | 特殊タイル                                                      | セカンダリ条件             | スコア上限  | ペナルティ設定          | アンロック条件 |
| ------------ | ---------- | ---------------------------------------------------------- | ------------------- | ------ | ---------------- | ------- |
| 6-1 ワープ基礎    | 5×5 / 中央固定 | ワープ {(0,0)↔(4,4)}                                          | 40 手以内              | 480 以下 | +3 / +2 / +1 / 0 | 5-8 クリア |
| 6-2 二重ワープ    | 5×5 / 中央固定 | ワープ {(0,4)↔(4,0), (1,1)↔(3,3)}                             | ペナルティ合計 2 以下        | 470 以下 | +3 / +2 / +1 / 0 | 6-1 クリア |
| 6-3 ワープ＋トグル  | 5×5 / 中央固定 | ワープ {(1,1)↔(3,3)} ＋ トグル {(2,2)}                            | 38 手以内              | 460 以下 | +3 / +2 / +1 / 0 | 6-2 クリア |
| 6-4 ワープ＋二度踏み | 5×5 / 中央固定 | ワープ {(0,4)↔(4,0)} ＋ 二度踏み {(2,2):2}                         | ペナルティ合計 1 以下        | 450 以下 | +3 / +2 / +1 / 0 | 6-3 クリア |
| 6-5 三重ワープ応用  | 5×5 / 任意   | ワープ {(0,0)↔(2,2), (1,4)↔(4,1), (0,4)↔(4,0)}                | 36 手以内              | 440 以下 | +3 / +2 / +1 / 0 | 6-4 クリア |
| 6-6 ワープ＋障害物  | 5×5 / 任意   | ワープ {(1,1)↔(3,3)} ＋ 障害物 {(2,2)}                            | ペナルティ合計 2 以下        | 430 以下 | +3 / +2 / +1 / 0 | 6-5 クリア |
| 6-7 複合転送課題   | 5×5 / 任意   | ワープ {(0,0)↔(4,4)} ＋ トグル {(1,1), (3,3)} ＋ 三重踏み {(2,2):3}    | 34 手以内              | 420 以下 | +3 / +2 / +1 / 0 | 6-6 クリア |
| 6-8 総合演習     | 5×5 / 中央固定 | ワープ {(0,4)↔(4,0), (1,3)↔(3,1)} ＋ トグル {(2,2)} ＋ 障害物 {(0,0)} | ペナルティ合計 2 以下＆32 手以内 | 410 以下 | +3 / +2 / +1 / 0 | 6-7 クリア |
...
* 複数マス移動カードを中心に、行動範囲の拡張を学ぶ章。途中でシャッフルマスを導入し、運要素のコントロールも体験させる。
* 盤面は全て5×5。スポーンは前半が中央固定、後半が任意スポーン。
* ペナルティ設定は標準（+3 / +2 / +1 / 0）。
...
ビジュアル: 通常カードの矢印を3連表示（→→→）。シアン系発光を採用。

アニメーション: 使用時に通過マスへ残光ラインを描画（SKShapeNode のフェードエフェクト）。

音響: 滑らかなスライド音。

シャッフルマス:

ビジュアル: 緑金色の円形矢印が回転するアイコン（SKSpriteNode アニメーションループ）。

アニメーション: 踏破時に小さな星屑が円状に飛ぶ（SKEmitterNode）。
...
| ----------------- | -------------------------------------------------------------------------------------- |
| `boardSize`       | 常に 5                                                                                   |
| `spawnRule`       | 7-1〜7-4: `.fixed(BoardGeometry.defaultSpawnPoint)` / 7-5〜7-8: `.chooseAnyAfterPreview` |
| `deckPreset`      | `.extendedWithMultiStepMoves`（新プリセット：複数マス移動カードを含む）                                     |
| `penaltySettings` | `.init(deadlock: 3, manualRedraw: 2, manualDiscard: 1, revisit: 0)`                    |
...
| ステージ              | 盤面 / スポーン  | 特殊タイル                                               | セカンダリ条件             | スコア上限  | ペナルティ設定          | アンロック条件 |
| ----------------- | ---------- | --------------------------------------------------- | ------------------- | ------ | ---------------- | ------- |
| 7-1 長距離移動基礎       | 5×5 / 中央固定 | なし                                                  | 40 手以内              | 500 以下 | +3 / +2 / +1 / 0 | 6-8 クリア |
| 7-2 八方向移動訓練       | 5×5 / 中央固定 | なし                                                  | 38 手以内              | 490 以下 | +3 / +2 / +1 / 0 | 7-1 クリア |
| 7-3 障害物回避訓練       | 5×5 / 中央固定 | 障害物 {(2,2)}                                         | ペナルティ合計 2 以下        | 480 以下 | +3 / +2 / +1 / 0 | 7-2 クリア |
| 7-4 直線＋二度踏み       | 5×5 / 中央固定 | 二度踏み {(1,1):2, (3,3):2}                             | 36 手以内              | 470 以下 | +3 / +2 / +1 / 0 | 7-3 クリア |
| 7-5 シャッフル基礎       | 5×5 / 任意   | シャッフル {(2,2)}                                       | 38 手以内              | 460 以下 | +3 / +2 / +1 / 0 | 7-4 クリア |
| 7-6 シャッフル＋長距離     | 5×5 / 任意   | シャッフル {(2,2)} ＋ 障害物 {(1,3)}                         | ペナルティ合計 2 以下        | 450 以下 | +3 / +2 / +1 / 0 | 7-5 クリア |
| 7-7 複合（シャッフル＋トグル） | 5×5 / 任意   | シャッフル {(2,2)} ＋ トグル {(1,1), (3,3)}                  | 36 手以内              | 440 以下 | +3 / +2 / +1 / 0 | 7-6 クリア |
| 7-8 総合演習          | 5×5 / 任意   | シャッフル {(0,0), (4,4)} ＋ 二度踏み {(2,2):2} ＋ 障害物 {(1,1)} | ペナルティ合計 2 以下＆34 手以内 | 430 以下 | +3 / +2 / +1 / 0 | 7-7 クリア |
...
* 新カード要素としてワープカードとスーパーワープカードを導入。位置制約を完全に解放し、最終的な総合戦略力を問う。
* ステージは5×5固定。全カード・全ギミックを活用し、最終章としての集大成を構成。
* ペナルティ設定は厳しめ（+3 / +2 / +1 / 0）だが、セカンダリ条件で合計2以下を要求するステージを多めに設定。

実装デザイン仕様

ワープカード:

ビジュアル: 紫白の二重リングが回転。中央から光粒が移動方向へ流れるアニメーション。

アニメーション: 使用時にSKEmitterNodeで光線を描画し、ターゲットマスにフェードイン転送。

音響: 短い転送サウンド。

スーパーワープカード:

ビジュアル: 背景に星空テクスチャ＋金のクラウンマーク。中央に多色発光リング。

アニメーション: 盤面全体を一瞬暗転し、全マスが光点で点滅。選択マスにスター爆発エフェクト。
...
| ----------------- | -------------------------------------------------------------------------------------- |
| `boardSize`       | 常に 5                                                                                   |
| `spawnRule`       | 8-1〜8-4: `.fixed(BoardGeometry.defaultSpawnPoint)` / 8-5〜8-8: `.chooseAnyAfterPreview` |
| `deckPreset`      | `.standardWithWarpCards`（新プリセット：ワープ・スーパーワープ含む）                                         |
| `penaltySettings` | `.init(deadlock: 3, manualRedraw: 2, manualDiscard: 1, revisit: 0)`                    |
...
| ステージ                   | 盤面 / スポーン  | 特殊タイル                                                          | セカンダリ条件             | スコア上限  | ペナルティ設定          | アンロック条件 |
| ---------------------- | ---------- | -------------------------------------------------------------- | ------------------- | ------ | ---------------- | ------- |
| 8-1 ワープカード基礎           | 5×5 / 中央固定 | なし                                                             | 40 手以内              | 480 以下 | +3 / +2 / +1 / 0 | 7-8 クリア |
| 8-2 スーパーワープ基礎          | 5×5 / 中央固定 | なし                                                             | ペナルティ合計 2 以下        | 470 以下 | +3 / +2 / +1 / 0 | 8-1 クリア |
| 8-3 ワープ＋障害物            | 5×5 / 中央固定 | 障害物 {(2,2)}                                                    | 38 手以内              | 460 以下 | +3 / +2 / +1 / 0 | 8-2 クリア |
| 8-4 スーパーワープ＋トグル        | 5×5 / 中央固定 | トグル {(1,1), (3,3)}                                             | 36 手以内              | 450 以下 | +3 / +2 / +1 / 0 | 8-3 クリア |
| 8-5 ワープ＋重踏             | 5×5 / 任意   | 二度踏み {(2,2):2} ＋ ワープ {(0,0)↔(4,4)}                             | ペナルティ合計 2 以下        | 440 以下 | +3 / +2 / +1 / 0 | 8-4 クリア |
| 8-6 デッキ混成（ワープ＋スーパーワープ） | 5×5 / 任意   | ワープ {(0,4)↔(4,0)}                                              | 34 手以内              | 430 以下 | +3 / +2 / +1 / 0 | 8-5 クリア |
| 8-7 任意スポーン総合           | 5×5 / 任意   | トグル {(2,2)} ＋ 障害物 {(1,1)} ＋ ワープ {(0,0)↔(4,4)}                  | ペナルティ合計 2 以下        | 420 以下 | +3 / +2 / +1 / 0 | 8-6 クリア |
| 8-8 最終試験               | 5×5 / 任意   | ワープ {(0,4)↔(4,0)} ＋ スーパーワープ使用必須 ＋ トグル {(1,3)} ＋ 二度踏み {(3,1):2} | ペナルティ合計 2 以下＆32 手以内 | 410 以下 | +3 / +2 / +1 / 0 | 8-7 クリア |

## QA チェック観点

* ワープカードとスーパーワープカードの選択UIと効果範囲が正しく反映されているかを確認。
* 任意スポーンステージでスーパーワープを使用した際に踏破判定が正確に行われるか検証。
* 既存ギミックとの干渉（特にトグル・多重踏破・障害物）で進行不能や誤判定が発生しないかを重点確認。
