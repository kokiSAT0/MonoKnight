...
試練塔の最高到達階は標準騎士/跳躍騎士別にローカル保存するが、旧標準、クラシカル、Daily、ハイスコア用 leaderboard ID は現行設定から削除済みであり、現行の塔プレイからスコア送信や自動サインイン促しは行わない。

## 現行設定

| 対応モード | ステータス | リファレンス名 | Leaderboard ID | 備考 |
| --- | --- | --- | --- | --- |
| 試練塔 | 未定 | 未定 | 未定 | 標準騎士/跳躍騎士を分けるかも含め、スコア式、対象フロア、提出タイミングを決めてから追加する。 |
