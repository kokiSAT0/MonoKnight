...
| UI | `UI/GameViewModel.swift` | 進行中 | `GameBoardBridgeViewModel`、補助型、controller 導入済みだが、session state、overlay state、flow handling、service bridge がまだ集中している | session UI state、result/transition handling、pause/timer policy を段階分割する | `GameViewModel.swift` がゲーム進行の窓口に絞られ、UI 状態管理や遷移責務が補助型へ整理されている | `MonoKnightAppTests/GameViewIntegrationTests.swift`、`MonoKnightAppTests/GameViewModelTests.swift` |
