...
| UI | `UI/GameViewModelSupport.swift` | 進行中 | `GameSessionPresentationSupport.swift`、`GameSessionInteractionSupport.swift`、`GameSessionLifecycleSupport.swift` へ helper type 群を分離し、本体は `GameViewModel` façade extension と action / lifecycle 入口に寄せた。選択状態・バナー・購読 binding・サービス橋渡しを責務単位では追いやすくなった | `GameViewModel` extension 自体の action surface をさらに整理し、必要なら result / pause / service bridge の入口も追加分割する | `GameViewModel.swift` と `GameViewModelSupport.swift` を読めば公開 façade は追えるが、support extension にはまだ action surface が集中している状態を脱しきってはいない | `MonoKnightAppTests/GameViewIntegrationTests.swift`、`MonoKnightAppTests/GameViewModelTests.swift`、`swift test` |
| Game | `Game/MoveCard.swift` | 進行中 | 第1段階として `MoveCard+Registry.swift` へ `directionalRayCards` / `standardSet` / `allCases`、`MoveCard+Presentation.swift` へ表示名・分類・アクセシビリティ寄りメタデータを移した。本体は case 定義、`MovePattern`、移動解決、利用判定に寄りつつある | 第2段階として `MovePattern` 解決ロジックと registry 初期化の支援をさらに分離する | registry / presentation は別責務化できたが、移動解決ロジックはまだ本体に集中している | `Tests/GameTests/DeckTests.swift`、`Tests/GameTests/GameCoreAvailableMovesTests.swift`、`Tests/GameTests/MoveCardPresentationTests.swift`、`swift test` |
...
1. `UI/GameViewModelSupport.swift` の action surface をさらに薄くし、support facade から result / pause / service bridge の責務を追いやすくする。
2. `Game/MoveCard.swift` の第2段階として、`MovePattern` 解決ロジックと registry 初期化補助の分離可否を見極める。
3. `UI/Theme/AppTheme.swift` は extension 単位の運用を継続し、テーマ調整時に責務逆流がないか監視する。
