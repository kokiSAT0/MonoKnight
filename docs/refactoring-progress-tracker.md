...
| UI | `UI/GameViewModel.swift` | 進行中 | `SessionUIState`、`ResultPresentationState`、input/core binding/session reset helper まで分割済みだが、service bridge と初期表示オーケストレーションがまだ本体に残る | service bridge、prepareForAppear、設定同期まわりを必要最小限へ絞る | `GameViewModel.swift` がゲーム進行の窓口と helper 調停中心になり、入力・購読・UI 後始末の詳細が補助型へ整理されている | `MonoKnightAppTests/GameViewIntegrationTests.swift`、`MonoKnightAppTests/GameViewModelTests.swift` |
