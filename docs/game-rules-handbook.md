...
- 目的地制モードでは、踏破状態とは別に **現在の目的地 1 マス** と **次の目的地 2 マス** を保持する。
- 盤面表示では、移動候補を矩形の枠、現在目的地を中央の塗り菱形、次目的地を小さな中央ドットとして描き分ける。
...
- スタンダードおよび刷新後キャンペーンでは任意の手動引き直しを廃止し、**フォーカス** に置き換える。
  - フォーカスは現在目的地へ近づきやすいカードを優先して手札と NEXT を再配布する。
...
| キャンペーン全般 | 0 | フォーカス | +1 | 0 | 目的地 4〜14 個でクリア。スター条件でフォーカス回数を評価。 |
...
- **無料フォーカスマス** (`TileEffect.freeFocus`): 移動完了後、現在目的地へ近づきやすい手札と NEXT に再配布する。通常フォーカスと異なり `focusCount` は増やさない。
- **カード温存マス** (`TileEffect.preserveCard`): その手で使用したカードを消費しない。移動、手数、目的地獲得は通常どおり処理する。
- **ドラフトマス** (`TileEffect.draft`): 使用カードを通常どおり消費した後、現在目的地へ近づきやすい手札と NEXT に再配布する。通常フォーカスと異なり `focusCount` は増やさない。
...
- **転換マス** (`TileEffect.targetSwap`): 移動完了後、目的地取得を解決してから現在目的地と NEXT 目的地の先頭を入れ替える。使用カードは通常どおり消費し、`focusCount` や `penaltyCount` は増やさない。目的地制でない場合は何もしない。
...
実験場はカード種類（標準・選択・レイ・ワープ・目的地補助・特殊マス補助）と特殊マス種類を保存付きで切り替えられる。キャンペーン投入前の組み合わせ検証に使う。

## 5. クリア条件とスコアリング

<!-- スコア計算やクリア判定の仕様を共有し、ランキング連携時の確認事項を整理 -->
- スタンダードモードは **目的地を 12 個獲得**するとクリア。キャンペーンは章ごとに **4〜14 個**の目的地を獲得するとクリア。`GameMode.CompletionRule.targetCollection(goalCount:)` で管理する。
  - 目的地は現在地と直前目的地を避ける。
  - 現在地からマンハッタン距離 2〜4 の候補を優先し、候補不足時のみ盤面全体から補完する。
  - 目的地に到達すると `capturedTargetCount` が増え、先読みキューから次の目的地を補充する。
...
- キャンペーンではスター条件として **セカンダリ目標**（手数・時間・フォーカス回数）と **スコア上限**（≦ または ＜）を組み合わせる。比較方式は `CampaignStage.ScoreTargetComparison` で管理。<!-- レギュレーションとスター判定の連携を明確化 -->
