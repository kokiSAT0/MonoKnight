...
final class MoveCardResolutionTests: XCTestCase {
    ...
    func testTargetAssistCardsRequireTargetPoint() {
        ...
        XCTAssertTrue(MoveCard.targetStep.resolvePaths(from: origin, context: context).isEmpty)
        XCTAssertTrue(MoveCard.targetKnight.resolvePaths(from: origin, context: context).isEmpty)
    ...
    func testTargetAssistCardsResolveApproachingCandidatesWithTargetPoint() {
        ...
        let stepDestinations = Set(MoveCard.targetStep.resolvePaths(from: origin, context: context).map(\.destination))
        ...
        let knightDestinations = Set(MoveCard.targetKnight.resolvePaths(from: origin, context: context).map(\.destination))
