...
final class GameSceneAccessibilityTests: XCTestCase {
    ...
    func testMultiStepPathHighlightUsesFillWithoutFrame() {
        ...
        XCTAssertFalse(pathStyle.fillColor.isEqual(SKColor.clear), "途中マスは薄い水色塗りでレイの通過範囲を示します")
        XCTAssertGreaterThan(destinationStyle.lineWidth, 0, "終点はタップ可能なレイ移動先として水色枠を持つ想定です")
    ...
    func testDarknessMoveCandidatesUseHighContrastFrames() {
        ...
        XCTAssertTrue(visibleBasicStyle.strokeColor.isNeutralGrayForTesting, "視界内の基本移動は直接移動として黒/グレー系の枠を維持します")
    ...
    func testDungeonBasicMoveUsesFrameAndDungeonMarkersAvoidTileFrames() {
        ...
        let directTwoStepPoint = GridPoint(x: 0, y: 1)
        let multipleMovePoint = GridPoint(x: 0, y: 3)
        let rayDestinationPoint = GridPoint(x: 1, y: 3)
        ...
        scene.updateHighlights([
            ...
            .guideMultipleCandidate: [multipleMovePoint],
            .guideMultiStepCandidate: [rayDestinationPoint],
        ...
        guard let directTwoStepStyle = scene.highlightStyleForTesting(
            kind: .guideDirectTwoStepCandidate,
            at: directTwoStepPoint
        ), let multipleMoveStyle = scene.highlightStyleForTesting(
            kind: .guideMultipleCandidate,
            at: multipleMovePoint
        ), let rayDestinationStyle = scene.highlightStyleForTesting(
            kind: .guideMultiStepCandidate,
            at: rayDestinationPoint
        ) else {
            XCTFail("直接移動/レイ移動の枠ノードを取得できません")
            return
        }
        ...
        XCTAssertTrue(basicMoveStyle.strokeColor.isNeutralGrayForTesting, "基本移動は直接移動として黒/グレー系の枠で示します")
        XCTAssertTrue(cardMoveStyle.strokeColor.isNeutralGrayForTesting, "1マスカードも直接移動として黒/グレー系の枠で示します")
        XCTAssertTrue(directTwoStepStyle.strokeColor.isNeutralGrayForTesting, "2マス直接移動もレイではなく黒/グレー系の枠で示します")
        XCTAssertTrue(multipleMoveStyle.strokeColor.isNeutralGrayForTesting, "桂馬/選択系も直接移動として黒/グレー系の枠で示します")
        XCTAssertFalse(rayDestinationStyle.strokeColor.isNeutralGrayForTesting, "レイ終点だけは直接移動とは別の水色系枠で示します")
    ...
    func testStarChartSurveyTowerThemeKeepsDungeonMarkerSemantics() {
        ...
        let multipleMovePoint = GridPoint(x: 0, y: 3)
        let rayDestinationPoint = GridPoint(x: 0, y: 2)
        let damageTrapPoint = GridPoint(x: 2, y: 2)
        let keyPoint = GridPoint(x: 3, y: 2)
        ...
        guard let basicMoveStyle = scene.highlightStyleForTesting(kind: .dungeonBasicMove, at: basicMovePoint),
              ...
              let multipleMoveStyle = scene.highlightStyleForTesting(kind: .guideMultipleCandidate, at: multipleMovePoint),
              let rayDestinationStyle = scene.highlightStyleForTesting(kind: .guideMultiStepCandidate, at: rayDestinationPoint),
        ...
        XCTAssertTrue(basicMoveStyle.strokeColor.isNeutralGrayForTesting, "新テーマでも基本移動は黒/グレー系に寄せます")
        XCTAssertTrue(singleMoveStyle.strokeColor.isNeutralGrayForTesting, "新テーマでも単方向カードは黒/グレー系に寄せます")
        XCTAssertTrue(directTwoStepStyle.strokeColor.isNeutralGrayForTesting, "新テーマでも2マス直接移動は黒/グレー系に寄せます")
        XCTAssertTrue(multipleMoveStyle.strokeColor.isNeutralGrayForTesting, "新テーマでも桂馬/選択系は黒/グレー系に寄せます")
        XCTAssertFalse(rayDestinationStyle.strokeColor.isNeutralGrayForTesting, "新テーマでもレイ終点は直接移動とは別色にします")
        XCTAssertEqual(directTwoStepStyle.lineWidth, singleMoveStyle.lineWidth, accuracy: 0.01, "新テーマの2マス直接移動は通常単方向と同じ直接移動枠で示します")
        XCTAssertEqual(directTwoStepStyle.glowWidth, singleMoveStyle.glowWidth, accuracy: 0.01, "新テーマの2マス直接移動は通常単方向と同じ発光強度に揃えます")
